        ...
        switch(checkPointPair(ix,iy,x,y)) {
          case SINGLE : {
            if(multix) {
              tmp[x[ix]] += trans;
              ap.push_front(x[ix]);
              int it = ix+1, rb = 1, tb = 1, rc = 1, tc = 0, l = multiPointsLen(it,x);
              POINT_T rect = rectMultiPoints<TYPE>(l, grid_size);
              while((it - ix <= l) && x.ref(ix) == x.ref(it)) {
                tmp[x[it]] = POINT_T(rc * grid_size, tc * grid_size);
                ap.push_front(x[it]);
                if(tc == tb) {
                  if(rc == 0) {
                    ++rb;
          ...
          case DOUBLE : {
            if(dir.ref(ix,iy) != TOP) {
              ...
              if(multiy) {
                tmp[y[iy]] += trans;
                ap.push_front(y[iy]);
                int it = iy+1, rb = 1, tb = 1, rc = 1, tc = 0, l = multiPointsLen(it,y);
                POINT_T rect = rectMultiPoints<TYPE>(l, grid_size);
                while((it - iy <= l) && y.ref(iy) == y.ref(it)) {
                  tmp[y[it]] = POINT_T(rc * grid_size, tc * grid_size);
                  ap.push_front(y[it]);
                  if(tc == tb) {
                    if(rc == 0) {
                      ++rb;
